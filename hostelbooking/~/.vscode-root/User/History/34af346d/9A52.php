...
ini_set('display_errors', 0);
  ...
  if(isset($_GET['fetch_rooms']))
  {
    ...
    $beds = ($guests['beds']!='') ? $guests['beds'] : 0;
    $chairs = ($guests['chairs']!='') ? $guests['chairs'] : 0;
    ...
    $room_res = select("SELECT * FROM `rooms` WHERE `beds`>=? AND `chair`>=? AND `status`=? AND `removed`=?",[$beds,$chairs,1,0],'iiii');
