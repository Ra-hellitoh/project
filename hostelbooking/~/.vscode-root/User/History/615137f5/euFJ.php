...
// Fetch user details
$user_query = "SELECT name, phonenum, pincode FROM user_cred WHERE id = ?";
$stmt_user = $con->prepare($user_query);
$stmt_user->bind_param("i", $CUST_ID);
$stmt_user->execute();
$stmt_user->bind_result($user_name, $user_phone, $user_pincode);
$stmt_user->fetch();
$stmt_user->close();
...
if ($existing_booking_count > 0) {
    echo "<script>alert('Error: You can only book once per semester.'); window.location.href='bookings.php';</script>";
    exit();
}
