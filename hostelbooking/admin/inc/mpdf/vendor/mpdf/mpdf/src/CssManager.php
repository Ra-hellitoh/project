...
class CssManager
{
		...
		if ($CSSstr) {
			...
			for ($i = 0; $i < $styles_count; $i++) {
				...
				foreach ($tagarr as $tg) {
					...
					if (preg_match('/NTH-chair\((\s*(([\-+]?\d*)N(\s*[\-+]\s*\d+)?|[\-+]?\d+|ODD|EVEN)\s*)\)/', $tg, $m)) {
						$tg = preg_replace('/NTH-chair\(.*\)/', 'NTH-chair(' . str_replace(' ', '', $m[1]) . ')', $tg);
					}
					...
					if (trim($tags[0]) === '@PAGE') {
						...
						if ($t) {
							...
							} elseif (preg_match('/^(' . $this->mpdf->allowedCSStags . ')\s*:NTH-chair\((.*)\)$/', $t, $m)) {
								$tag = $m[1] . '>>SELECTORNTHchair>>' . $m[2];
	...
	// mPDF 6
		...
		// STYLESHEET nth-chair SELECTOR e.g. tr:nth-chair(odd)  td:nth-chair(2n+1)
		if ($tag === 'TR' && isset($p) && $p) {
			foreach ($p as $k => $val) {
				if (preg_match('/' . $tag . '>>SELECTORNTHchair>>(.*)/', $k, $m)) {
					$select = false;
					if ($tag === 'TR') {
						$row = $this->mpdf->row;
						...
						if (preg_match('/(([\-+]?\d*)?N([\-+]\d+)?|[\-+]?\d+|ODD|EVEN)/', $m[1], $a)) { // mPDF 5.7.4
							$select = $this->_nthchair($a, $row);
						}
					} elseif ($tag === 'TD' || $tag === 'TH') {
						if (preg_match('/(([\-+]?\d*)?N([\-+]\d+)?|[\-+]?\d+|ODD|EVEN)/', $m[1], $a)) { // mPDF 5.7.4
							$select = $this->_nthchair($a, $this->mpdf->col);
						}
					}
					if ($select) {
						$this->_mergeCSS($p[$tag . '>>SELECTORNTHchair>>' . $m[1]], $t);
					}
				}
			}
		}
		...
		if ($tag === 'TR' || $tag === 'TD' || $tag === 'TH') {
			foreach ($this->CSS as $k => $val) {
				if (preg_match('/' . $tag . '>>SELECTORNTHchair>>(.*)/', $k, $m)) {
					...
					if ($select) {
						$zp = $this->CSS[$tag . '>>SELECTORNTHchair>>' . $m[1]];
						if ($tag === 'TD' || $tag === 'TH') {
							$this->setBorderDominance($zp, 9);
						}
						if (is_array($zp)) {
				...
				if ($tag === 'TR' || $tag === 'TD' || $tag === 'TH') {
					foreach ($this->tablecascadeCSS[$this->tbCSSlvl - 1] as $k => $val) {
						if (preg_match('/' . $tag . '>>SELECTORNTHchair>>(.*)/', $k, $m)) {
							...
							if ($select) {
								$this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1][$tag . '>>SELECTORNTHchair>>' . $m[1]], $p, false, 9);
							}
						}
					}
				}
	...
	// mPDF 5.7.4   nth-chair
	function _nthchair($f, $c)
	{
